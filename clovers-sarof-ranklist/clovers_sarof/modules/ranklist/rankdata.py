from clovers_sarof.core.account import AccountBank, UserBank, Group, User, Account, col


def _rank_user_extra_all(key: str, limit: int):
    query = User.select().where(col(User.extra).contains({key: None})).order_by(col(User.extra[key]).desc()).limit(limit)
...
def _rank_user_extra_group(key: str, group_id: str, limit: int):
    query = (
        Account.select()
        .where(Account.group_id == group_id, col(User.extra).contains({key: None}))
        .order_by(col(Account.extra[key]).desc())
        .limit(limit)
    )

    def func(account: Account):
...
def _rank_user_bank_all(item_id: str, limit: int):
    query = UserBank.select().where(UserBank.item_id == item_id).order_by(col(UserBank.n).desc()).limit(limit)
...
def _get_account_bank_all(item_id: str, limit: int):
    query = (
        ...
        .order_by(col(AccountBank.n * Group.level).desc())
        .limit(limit)
    )
